...
{
    public OmerBarLevSupplier(string name, string email, List<Product> products) : base(name, email, products)
    {
    }
    ...
    public override void AddProductsToMachine(Machine machine, Product product, int amount)
    {
        ...
        {
            machine.HotDrinks.Add((HotDrink)product, product.Amount);
        }
